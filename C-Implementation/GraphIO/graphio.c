...
void writeGraph(GraphInfo gi) {
    ...
    for(int x=0;x<numVerts(gi->graph);x++){
        char* vert_string= gi->vertnames[x];
        int vert = vertexNum(gi, gi->vertnames[x]);
        int* succ = successors(gi->graph, vert);

        for(int y=0;succ[y]!=-1;y++){
            printf("%s %s %.2f\n", vert_string,gi->vertnames[succ[y]],edge(gi->graph,x,succ[y]));
        }
    }
